...
class LockClient(Sender):
    ...
    def __init__(self, redpitayas, FSR = 906, DIR = None):
        ...
        self.RPs = redpitayas # dictionary of RP_client objects with respective settings
        self.masters = []
        self.monitors = dict()
    ...
    def find_slave_RPs(self, master_RP):
        ...
        for key, val in self.RPs.items():
            if val.settings['Master'] == master_RP and val.mode in ['lock','monitor']:
                RPs.append(key)
        RPs.append(master_RP) # add the master_RP, since it is also associated with the same cavity. its the last entry.
        return RPs
    ...
    def _apply_to_monitor(func):
        ...
        def inner(self, RP, *args, **kwargs):
            result = func(self, RP, *args, **kwargs) # call the function first! --> settings are changed
    ...
    def _check_cavity_scanned(func):
        ...
        def inner(self, RP, *args, **kwargs): # IMPORTANT: at least RP as argument is expected!
            if self.check_cavity_scanned(RP):
                return func(self, RP, *args, **kwargs) # output of the function shall be returned!
            else:
    ...
    def _check_for_loop(func):
        ...
        def inner(self, RP, *args, **kwargs):  # IMPORTANT: at least RP as argument is expected!
            if self.RPs[RP].loop_running:
                print(f'Loop currently running on {RP}! Stop it before running this function!')
                return np.array([])
            else:
                return func(self, RP, *args, **kwargs)
        return inner
    
    def check_cavity_monitored(self, RP):
        '''
        Used to check if the cavity associated with RP is currently monitored.
        '''
        master = self
    ...
    def check_cavity_scanned(self, RP):
        ...
        if type(master) == str: # if not master RP, then check if cavity is scanned first!
            if self.RPs[master].loop_running:
                return True
            else: 
    ...
    def check_new_settings(self, RP, laser, key, val):
        ...
        if key == 'range':   
            if not check_range(laser, val, self.get_current_dec(RP)):
                print(f"range {val} will not work! pay attention to the order and limits!")
                return False
        ...
        if key == 'lockpoint':
            ...
            if not check_lockpoint(laser, R, val):
                print(f"lockpoint {val} is not valid. Either not float or outside of range {R}! (second range for Master!)")
                return False
        if key == 'enabled':
    ...
    def new_range_new_lp(self, RP, laser, R):
        ...
        if not self.check_range_contains_lp(RP, laser, R):
            query = input(f"range {R} does not contain the current lockpoint. If this is intended, input new lockpoint here (non-valid value to cancel):\n")
            if check_lockpoint(laser, R, float(query)): # if valid lockpoint is chosen, apply it.
    ...
    def _check_update_setting(func):
        ...
        def inner(self, RP, laser, key, val):
            if laser not in self.RPs[RP].settings: # do not accidently add another output to the redpitaya!
                print(f"There is no laser {laser} in the settings!")
                return
            elif type(self.RPs[RP].settings[laser]) == dict:
                if key not in self.RPs[RP].settings[laser]: # do not accidently add another setting!
                    print(f"{key} does not exist in settings!")
                    return
            if not self.check_new_settings(RP, laser, key, val):
                return
            return func(self, RP, laser, key, val) # if every check worked out, finally run the function!
        return inner
    ...
    def start_loop(self, RP, action):
        ...
        t = threading.Thread(target = self.send, args = (RP, action), kwargs = dict(loop_action=True))
        t.daemon = True
        t.start()
    ...
    def _load_default_settings(self, RP):
        ...
        '''
        print(f'No settings for {RP} found, creating new setting file based on defaults.')
        ...
        if self.RPs[RP].mode == 'scan':
            ...
            if len(self.masters) > 0:
                default['Master'] = self.masters[0] # reference the first cavity by default
            else:
                default['Master'] = 'Cav'
    ...
    def change_cavity(self, RP, RP_master):
        ...
        --> change the master cavity, where the laser corresponds to.
        '''
    ...
    def update_setting(self, RP, laser, key, val):
        ...
        self.RPs[RP].settings[laser][key] = val # update the setting with the considered value
        self.save_settings(RP) # save the updated settings in an external json file for later!
        settings = self.retrieve_settings(RP)
        return self.send(RP, 'update_settings', value = settings) # then, send the actual settings!
    ...
    def save_settings(self, RP):
        ...
        RP : str
            Key of the RedPitaya in question.
        '''
        with open(Path(self.DIR, f'{RP}.json'), 'w') as file: # the file is called after the RP key!
    ...
    def retrieve_settings(self, RP):
        ...
        if RP_.mode != 'scan':
            ...
            settings['Master'] = deepcopy(self.RPs[RP_.settings['Master']].settings['Master'])
        else:
            settings = deepcopy(RP_.settings)
        ...
        for key in settings:
            ...
            if key == 'Master':
                ...
                settings[key]['range'] = [[ms2index(x, dec) for x in r] for r in R] # nested list comprehension
            else:
                R = settings[key]['range']
    ...
    def retrieve_monitor_settings(self, master_RP):    
        ...
        '''
        settings = {}
        RPs = self.find_slave_RPs(master_RP) # collect all redpitayas associated with master_RP
        for key in RPs: # key: RP identifier
            for val_key, val_val in self.RPs[key].settings.items(): # val_key: Laser identifier 
                # find master settings
                s = deepcopy(self.retrieve_settings(key))[val_key]
                if key in self.masters and val_key == 'Master':
                    settings[val_key] = s
                elif val_key != 'Master' and self.RPs[key].mode == 'lock': # each other case should contain laser locking settings
    ...
    def get_current_dec(self, RP):
        ...
        for key in settings:
            if key == 'Master' and type(settings[key]) != str: # RP is scanning the cavity
                dec0 = settings[key]['dec']
            else:
                dec0 = self.RPs[settings['Master']].settings['Master' ]['dec'] # dec from associated master settings
        return dec0
    ...
    def rescale_settings(self, RP, c):
        ...
        for key in settings:
            if key == 'Master' and type(settings[key]) != str : # range for master settings
                R = settings[key]['range']
                settings[key]['range'] = [[x*c for x in r] for r in R] # nested list comprehension
    ...
    def set_dec(self, master_RP, dec):
        ...
        if not check_dec(dec):
            return
        RPs = self.find_slave_RPs(master_RP) #  first, find the redpitayas associated with master_RP
        for RP in RPs: # then, get the recent dec setting and use it to rescale the relevant settings
            dec0 = self.get_current_dec(RP)
            self.rescale_settings(RP, dec/dec0) # adjust the settings accordingly!
        self.RPs[master_RP].settings['Master']['dec'] = dec  # afterwards, overwrite the setting for the master laser!
        for RP in RPs: 
            self.save_settings(RP)# finally save all the settings to the json files!
            self.send(RP, 'set_dec', value=dec) # ... and send the setting to the redpitayas!
    ...
    def start_error_monitor(self, RP, tmin = 10e-3):
        ...
        if RP in self.monitors:
            ...
            self.p = mp.Process(target = monitor_errors, args = (mon['running_err'], self.RPs[RP], mon['queue_err'], settings), kwargs=dict(FSR = self.FSR, tmin = tmin))
            self.p.daemon = True
            self.p.start()
    ...
    def start_monitor(self, RP):
        ...
        if RP in self.monitors:
            ...
            settings = self.retrieve_monitor_settings(master_RP)
            print('Starting background process')
            self.p = mp.Process(target = monitor, args = (mon['running'], self.RPs[RP], mon['queue'], settings))
            self.p.daemon = True
            self.p.start()
            print('monitoring process started')
    ...
    def send(self, RP, action, value='Hello world!', loop_action = False):
        ...
        if RP in self.RPs:
            ...
            return self.RPs[RP].send(self, action, value=value, loop_action=loop_action, loop=loop) # call the respective send method.
        else:
    ...
    def connect_all(self):
        ...
        print('connecting...')
        for RP in self.RPs:
            t = threading.Thread(target = self.RPs[RP].start_host_server, daemon = True) # collect all connection functions in threads
            t.start()
            t.join() # join the thread such that the main program waits for all redpitayas to connect!
        sleep(5) # sleep for 5 seconds while each redpitaya loads the respective libraries.
...
class Monitor(Sender):
    ...
    def stop_monitor(self, event):
        self.monitor_running.value = False # this is used to stop the monitor if figure is closed!
    ...
    def setup_monitor(self):
        ...
        plt.show(block = False) # this is required in order to see the figure when using multiprocessing
        ...
        self._fig.canvas.mpl_connect('close_event', self.stop_monitor) # close the monitor whenever the plot is closed!
    ...
    def plot_settings(self):
        ...
        for key, val in self.settings.items():
            if val['enabled']:
                if key == 'Master':
                    c = 'k'
                    for R in val['range']:  # indicate ranges using axvspan and setpoints using vlines
                        ref = plt.axvspan(self.times[R[0]], self.times[R[1]], alpha = 0.2, facecolor=c)
                    ...
                    ref = plt.axvspan(self.times[R[0]], self.times[R[1]], alpha = 0.2, facecolor=c)
                    self._setrefs.append(ref)
                ref = plt.vlines([val['lockpoint']], -1, 1, color = c, label = self.create_label(key))
                self._setrefs.append(ref)
                i += 1
    ...
    def update_settings(self, settings):
        ...
        self.settings = settings # update the settings attribute for the following functions!
...
class ErrorMonitor(Sender):
    ...
    def setup_monitor(self):
        ...
        self._fig.canvas.draw()
        plt.show(block = False) # this is required in order to see the figure when using multiprocessing
        self._bm = BlitManager(self._fig.canvas, animated_artists = list(self._lines.values()))    # for blitting only
        self._fig.canvas.mpl_connect('close_event', self.stop_monitor) # close the monitor whenever the plot is closed!
    ...
    def save_errors(self, filename):
        ...
        with open(f'{filename}.json', 'w') as file: # dump the data into a json file, since it is a dictionary.
        ...
        for key in self.errs:
            if new_errs == 'skipped':
                ...
                if key in new_errs: # if the error is measured on the redpitaya:
                    self.errs[key].append(new_errs[key]*self.FSR) # update the error value
                else:
    ...
    def update_monitor(self):
        ...
        for key, l in self._lines.items():
            if len(self.times) >= 300:   # only plot last 100 points --> choose first index accordingly
                i0 = -300
            else:
                i0 = 0
            ...
            if len(self.times)>2:
                self._ax.set_xlim(self.times[i0], self.times[-1])                        # updating the xlim accordingly, since new points get added
                self._ax.relim()
