...
from pathlib import PurePosixPath, Path # just PosixPath did not work on our Windows machine...
import os
import sys
...
class Sender:
    ...
    def event_loop(self):
        ...
        try:
            pass
            while True:
                if not self.sel.get_map(): # check whether there is something registered on the selector!
                    sleep(0)
                    pass
                else:
                    events = self.sel.select(timeout=1)  # select returns list of tuples, each for a socket.
                    for key, mask in events: # each tuple contains key and mask --> selectorkey | eventmask --> this iterates through each socket connection!
                        message = key.data   # retrieve message
                        if key.data != None:
                            try:
                                # this is for quicker data acquisition if multiple traces are subsequently captured
                                if self.mode == 'monitor':  # increase size if more data needs to be transferred at a time
                                    message.buffersize = int(2**18)
                                else: 
                                    message.buffersize = int(2**12)
                                message.process_events(mask) # Process the event according to the mask! --> read or write.
                            except Exception:
                                print(
                    ...
                    if not self.running: # if running is set to False using stop_event_loop, the event_loop stops.
                        break
    ...
    def start_event_loop(self):
        ...
        if not self.running: # only if the event loop is not running already!
            self.el_thread = threading.Thread(target=self.event_loop) # use threading to run the event_loop in the background
            self.el_thread.daemon = True # a daemon thread will shut down if the program exits.
...
class RP_connection:
    ...
    def reboot(self):
        ssh = paramiko.SSHClient() # save the ssh client as attribute in order to close it later!
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy) # used for not yet known hostnames!
        ssh.connect(self.addr[0], port = 22, username='root', password = 'root') # the standard port 22 is hardcoded here.
    ...
    def upload_current(self):
        ...
        for key in ['Lock', 'Lib', 'Run', 'Peaks']:
            path = PurePosixPath("/home/jupyter/RedPitaya", filenames[key]) # path on RP
            localpath = filepaths[key] # path on PC
            if key == 'Run': # This is the one file on the redpitaya requiring the hostname!
                # read the file
                ...
                lines[5] = f"Lock = RP_Server(host, port, 5065, RP_mode = '{self.mode}')\n"
    ...
    def start_host_server(self):
        ...
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy) # used for not yet known hostnames!
        ssh.connect(self.addr[0], port = 22, username='root', password = 'root') # the standard port 22 is hardcoded here.
        ssh.exec_command("python3 /home/jupyter/RedPitaya/RunLock.py")  #run the script which initiates socket server!
        ssh.close() # close the ssh connection afterwards, otherwise there are problems with multiprocessing...
        self.connected = True
        return 'connected'
    ...
    def send(self, Sender, action, value = 'Hello World!', loop_action = False, loop=False): # Sender contains the event_loop that handles the multiple communications
        # used to send commands (so-called actions) to the redpitaya. e.g. send("echo")
        if Sender.running: # check if event_loop is running!
            
            request = self.create_request(action, value) # this is used to work with the RealPython socket example
            ...
            event_state = selectors.EVENT_READ | selectors.EVENT_WRITE     # either read or write
                ...
                if loop_action and self.loop_running:
                    if self.lsock == None:
                        if Sender.sel.get_key(sock).events & selectors.EVENT_READ:
                            ...
                            sleep(2)
                            self.lsock = self.connect_socket(laddr) # the second socket connection has to be established!
                            sleep(0.5)
                            try: 
