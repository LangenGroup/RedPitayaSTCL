...
In this script, a feed-forward on the TransferCavity stabilization is implemented,
using an additional RedPitaya. In our Transfer Cavity design, we have piezos on
both mirrors. One for the scanning and active stabilization in the default
STCL scheme. The other one is used to apply an offset using an additional
powersupply in order to offset the cavity length in order to choose an appropriate
range for locking. The same offset piezo can also be regulated according to the
active stabilization in the STCL scheme. That way, the scanning piezo stays in
the same voltage range and effects due to piezo nonlinearity can be greatly reduced.

Note that the following instructions use an OWON power supply, and the feed-forward
redpitaya is controlled via the SCPI server provided by RedPitaya. For that reason,
the redpitaya_scpi script is also present in this same directory
...
V_set = (
    -4
)  # Set voltage [V] of the scanning piezo to be stabilized with the offset piezo
piezo_limits = [5, 60]  # Voltage limits [V] of the offset piezo used for PID
PID_limits = dict(P=0.1, I=0.0, D=0.0, limit=[-0.15, 0.15])
